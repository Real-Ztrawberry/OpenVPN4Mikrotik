...
function installQuestions() {
	echo ""
	echo "Welcome to the OpenVPN Server 4 Mikotik Clients installer!"
	echo "----------------------------------------------------------"
	echo ""
	echo "This script is available at the git repository at: https://github.com/Real-Ztrawberry/openvpn4mikrotik"
	echo ""
	echo "******************"
	echo "*IMPORTANT NOTICE*"
	echo "******************"
	echo ""
	echo "This script was modified to ensure a successfull installation of an OpenVPN"
	echo "server to be used by Mikrotik RouterOS clients. Compatibility with RouterOS"
	echo "is the goal but client configurations generated by this script can still be"
	echo "used by Android, Windows, Linux etc. OpenVPN clients."
	echo ""
	echo "The following changes were made to ensure RouterOS clients can connect:"
	echo "   - Removal of compression (LZ0, LZ4...)"
	echo "   - Removal of TLS-Auth and TLS-Crypt options"
	echo "   - Cipher: Only allow CBC options and removal of GCM options (data channel)"
	echo "*****************************************************************************"
	echo ""
	echo "The original script is available in the git repository at:"
	echo "https://github.com/angristan/openvpn-install"
	echo ""
	read -p "Press enter to continue..."
	clear
	echo "A few details are required before we start the installation"
	...
	echo "UDP is fasterand unless it is not available, you shouldn't use TCP."
	echo "   1) UDP"
	echo "   2) TCP"
	...
	echo ""
	echo "The RouterOS does not implement compression. Therefore we will disable it."
	echo "The VORACLE attack also makes use of it, so a blessing in disguise."
	echo ""
...
#		TLS_SIG="1" # tls-crypt
		...
		echo "   1) AES-128-CBC (recommended for performance)"
		echo "   2) AES-192-CBC"
		echo "   3) AES-256-CBC"
		until [[ $CIPHER_CHOICE =~ ^[1-3]$ ]]; do
			read -rp "Cipher [1-3]: " -e -i 1 CIPHER_CHOICE
		done
		...
		echo ""
		echo "PLEASE NOTE:"
		echo "Since the current Mikrotik implementation of OpenVPN does not support tls-auth and tls-crypt"
		echo "this script has been modified to remove the selection"
		echo ""
...
function manageMenu() {
	echo "Welcome to the OpenVPN Server 4 Mikotik Clients User Manager."
	echo "This script is available at the git repository at: https://github.com/Real-Ztrawberry/openvpn4mikrotik"
	echo ""
